...
const list = requirePermission('dashboard:read', async (req, res, data, params) => {
  const pg = pagination.parsePaginationParams(req.url);
  const result = pagination.applyQuery(data.dashboards, {
    filters: pg.filter,
    search: pg.search,
    searchFields: ['name'],
    sort: pg.sort,
...
const create = requirePermission('dashboard:write', async (req, res, data, params) => {
  const body = await readBody(req);
  const item = instrument(data, body.instrumentId);
  if (!body.name?.trim() || !item) return json(res, 422, { error: 'A dashboard name and valid instrument are required.' });
  ...
  const dashboard = {
    ...
    name: body.name.trim(),
    widgets: [{ type: 'bar', instrumentId: item.id, dimension: body.dimension, measure: 'response_count' }],
    createdAt: new Date().toISOString(),
    createdBy: req.user.id
  };
  data.dashboards.unshift(dashboard);
  audit(data, 'CREATE', 'dashboard', dashboard.id, { name: dashboard.name, instrumentId: item.id });
  write(data);
